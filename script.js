...
// ======== POPUP BANNER: Kombinasi "Tab baru" + "Kembali dari buy-macbook >=5 menit" ========
// Penanda versi storage agar aman kalau logika berubah.
const POPUP_VERSION = 'v2';

// ---- Konfigurasi dasar ----
const FIVE_MINUTES_MS = 5 * 60 * 1000;
const LS_KEYS = {
  // Akumulasi waktu aktif di halaman buy (dalam ms).
  buyActiveMs: `buyActiveMs_${POPUP_VERSION}`,
  // Flag bahwa popup sudah ditampilkan sekali karena syarat 5 menit terpenuhi, untuk "hari ini".
  popupShownAfter5MinDate: `popupShownAfter5MinDate_${POPUP_VERSION}`,
};
const SS_KEYS = {
  // Penanda tab (per-tab). Kalau belum ada -> berarti tab baru.
  tabId: `tabId_${POPUP_VERSION}`,
  // Status "apakah popup di-tab-ini sudah pernah ditutup?" (per-tab)
  dismissedThisTab: `dismissedThisTab_${POPUP_VERSION}`,
};

// ---- Utilitas kecil ----
function todayStr() {
  // Format YYYY-MM-DD agar gampang dibanding harian
  return new Date().toISOString().slice(0, 10);
}

// Dapatkan elemen modal (hanya ada di index.html)
function $modal() { return document.getElementById('promoModal'); }
function $closeBtn() { return document.getElementById('promoClose'); }
...
function isIndexPage() {
  ...
  // Netlify/hosting statis bisa serve "/" sebagai index
  return p === '/' || p.endsWith('/index.html');
}
function isBuyMacbookPage() {
  return location.pathname.endsWith('/buy-macbook.html');
}

// ---- Mekanisme "tab baru" ----
// Jika sessionStorage belum punya tabId, berarti tab baru dibuka -> set tabId sekarang.
function ensureTabId() {
  if (!sessionStorage.getItem(SS_KEYS.tabId)) {
    sessionStorage.setItem(SS_KEYS.tabId, String(Math.random()));
  }
}

// ---- Mekanisme akumulasi waktu aktif di halaman buy ----
// Kita hitung waktu saat tab "visible" agar lebih akurat.
// Interval ringan setiap 5 detik untuk menambah akumulasi ketika tab sedang kelihatan.
let _buyTickTimer = null;
function startBuyActiveTimerIfNeeded() {
  if (!isBuyMacbookPage()) return;

  // Pastikan ada nilai awal
  if (!localStorage.getItem(LS_KEYS.buyActiveMs)) {
    localStorage.setItem(LS_KEYS.buyActiveMs, '0');
  }

  function tick() {
    // Hanya tambah waktu kalau tab sedang terlihat
    if (document.visibilityState === 'visible') {
      const cur = Number(localStorage.getItem(LS_KEYS.buyActiveMs) || '0');
      localStorage.setItem(LS_KEYS.buyActiveMs, String(cur + 5000)); // +5 detik
    }
  }

  // Mulai interval 5 detik
  _buyTickTimer = setInterval(tick, 5000);

  // Jika user tutup/refresh halaman, hentikan interval (rapi)
  window.addEventListener('beforeunload', () => {
    if (_buyTickTimer) clearInterval(_buyTickTimer);
  });
}

// ---- Kapan popup dibuka di index? ----
// 1) "Tab baru": popup SELALU tampil saat membuka index di tab baru (sekali per tab).
// 2) "Kembali dari buy >=5 menit":
//    - Jika akumulasi waktu di buy sudah >= 5 menit,
//      dan untuk "hari ini" belum pernah memicu popup karena aturan ini,
//      maka tampilkan popup.
function shouldOpenPopupOnIndex() {
  // Kalau belum ada modal (bukan di index), jelas false.
  if (!isIndexPage() || !$modal()) return false;

  // RULE 1: Tab baru -> tampilkan popup (kecuali user sudah menutup di tab ini).
  const isNewTab = !sessionStorage.getItem(SS_KEYS.tabId);
  if (isNewTab) return true;

  // RULE 2: Kembali dari buy >= 5 menit (sekali per hari).
  const activeMs = Number(localStorage.getItem(LS_KEYS.buyActiveMs) || '0');
  const alreadyShownToday = localStorage.getItem(LS_KEYS.popupShownAfter5MinDate) === todayStr();

  if (activeMs >= FIVE_MINUTES_MS && !alreadyShownToday) {
    return true;
  }
...
// ---- Tindakan setelah popup tampil karena RULE 2 ----
function markShownAfter5MinToday() {
  localStorage.setItem(LS_KEYS.popupShownAfter5MinDate, todayStr());
}

// ---- Inisialisasi ----
document.addEventListener('DOMContentLoaded', () => {
  // 0) Tandai tabId dulu untuk mendeteksi "tab baru"
  const wasNewTab = !sessionStorage.getItem(SS_KEYS.tabId);
  ensureTabId();

  // 1) Kalau di halaman buy, mulai timer akumulasi 5 menit
  startBuyActiveTimerIfNeeded();

  // 2) Kalau di index, putuskan apakah harus buka popup
  if (isIndexPage() && $modal()) {
    // Event penutupan (X, klik overlay, Esc)
    if ($closeBtn()) {
      $closeBtn().addEventListener('click', () => {
        sessionStorage.setItem(SS_KEYS.dismissedThisTab, '1');
        closePromoModal();
      });
    }
    const modalEl = $modal();
    ...
    // Logika tampil:
    // a) Jika tab benar-benar BARU (wasNewTab = true) -> tampil (RULE 1)
    // b) Jika bukan tab baru, cek RULE 2 (>=5 menit di buy dan belum ditampilkan hari ini)
    const dismissedThisTab = !!sessionStorage.getItem(SS_KEYS.dismissedThisTab);
    if (wasNewTab && !dismissedThisTab) {
      openPromoModal(); // RULE 1
      // Tidak menandai "shownAfter5MinToday" karena alasan tampilnya bukan RULE 2
    } else if (shouldOpenPopupOnIndex() && !dismissedThisTab) {
      openPromoModal(); // RULE 2
      markShownAfter5MinToday();
    }
  }
});
